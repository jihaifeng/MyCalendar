...
public class MyCalendar extends FrameLayout implements NestedScrollingParent, ValueAnimator.AnimatorUpdateListener, OnWeekCalendarChangedListener, OnMonthCalendarChangedListener {
    ...
    private static int STATE = 200;//默认显示周日历
    private int weekHeight;//周日历的高度
    private int monthHeight;//月日历的高度,是日历整个的高度，并非是月日历绘制区域的高度
    ...
    private void autoScroll(int startMonth, int endMonth, int startChild, int endChild) {
        ...
        childViewValueAnimator.setDuration(240);
    ...
    public void toWeek() {
        if (STATE == MONTH) {
            ...
            autoScroll(0, -monthCalendarOffset, monthHeight, weekHeight);
        }
    }
